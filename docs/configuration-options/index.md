...
Only consumers that implement the extension benefit. Ones that do not will resolve a position inside a range to the range's start rather than the true position, so enabling this is a regression for tooling that has not been taught the format.
